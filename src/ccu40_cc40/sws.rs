...
pub type SCMU_W<'a, REG, const O: u8> = crate::BitWriter<'a, REG, O>;
#[doc = "Field `SCMD` writer - Compare match while counting down set"]
pub type SCMD_W<'a, REG, const O: u8> = crate::BitWriter<'a, REG, O>;
...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn scmu(&mut self) -> SCMU_W<SWS_SPEC, 2> {
        SCMU_W::new(self)
    }
    #[doc = "Bit 3 - Compare match while counting down set"]
    #[inline(always)]
    #[must_use]
    pub fn scmd(&mut self) -> SCMD_W<SWS_SPEC, 3> {
        SCMD_W::new(self)
    }
